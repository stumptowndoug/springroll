# Historical TODO snapshot — September 5, 2026

Archived when the roadmap was refocused on the five desktop-alpha priorities. Statuses below reflect the board at that time, not active assignments. Use [the current board](../../TODO.md) for all new work; older incomplete items are deferred unless explicitly brought back into it. Completed records are retained here unchanged.
...
- [ ] Audit missed-run semantics and real Mac sleep/wake recovery before packaging
  - Priority: do this first, then the existing macOS packaging proof, then packaged-app reliability and public release readiness.
  - Clarify that current `skip_to_next` still claims one overdue run, while `catch_up` advances from the overdue schedule; agree on intended user-facing behavior.
  - Test overnight/weekend sleep, app quit/relaunch, unavailable networking, multiple missed occurrences, paused recipes, approval-waiting runs, and timezone/DST changes.
  - Verify explicit late/skipped reporting, no unintended duplicate side effects, and the correct next scheduled run.

- [ ] Refresh the source-alpha release gate against the current code
  - Re-scan history since the earlier 282-commit check and review tracked screenshots, fixtures, and design artifacts for private data.
  - Update README capabilities and security/limits documentation after the cleanup changes; include the production build in required CI.
  - Reuse the existing source-alpha checklist for clean-account onboarding, release screenshots, main-branch integration, and publication approval.
...
- [x] Inspect missed-run handling and recommend the next desktop-alpha priorities
  - Four occurrence/persistence tests passed. Current skip mode still claims one overdue run; existing offline-registry coverage does not substitute for real Mac sleep/wake acceptance. Prioritized missed-run audit before packaging.

- [x] Review the next release milestone and compare source-alpha readiness with hosted execution
  - Recommended source-run alpha first, a separate macOS packaging proof next, and a bounded BYOK hosted-recipe proof before any multi-user hosted launch. Direction remains subject to user choice.
