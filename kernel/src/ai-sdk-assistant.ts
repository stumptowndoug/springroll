...
const defaultSystem = [
  ...
  "When the user asks to connect a service, use Springroll's connection-research tool first. If it cannot verify a compatible remote connector, use web search and direct fetch to inspect official provider documentation, the official source repository, and package metadata. When those sources identify a local npm MCP package and its credential environment variable, submit that evidence through Springroll's local-MCP proposal tool so the host can verify and pin it; otherwise explain the verified manual or API path without inventing a server or setup state.",
